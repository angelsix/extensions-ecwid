...
namespace Ecwid
{
    public partial class EcwidClient
    {
        ...
        /// <inheritdoc />
        public Task<UpdateStatus> MarkOrderAsShippedAsync(int orderId, CancellationToken cancellationToken)
        {
            return PutApiAsync<UpdateStatus>(GetUrl($"orders/{orderId}"), new { orderNumber = orderId, fulfillmentStatus = "SHIPPED" }, cancellationToken);
        }
